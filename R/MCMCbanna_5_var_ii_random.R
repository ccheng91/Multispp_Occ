...
sitecov$pas <- as.character(sitecov$pas)
sitecov$pas[which(sitecov$pas == "mengla")] <- 3
sitecov$pas[which(sitecov$pas == "mangao")] <- 2
sitecov$pas[which(sitecov$pas == "menglun")] <- 4
sitecov$pas[which(sitecov$pas == "bulangshan")] <- 1
sitecov$pas[which(sitecov$pas == "mengsong")] <- 5
sitecov$pas[which(sitecov$pas == "nabanhe")] <- 6
sitecov$pas <- as.numeric(sitecov$pas)

park.ind <- sitecov$pas 
nsite <- max(park.ind)  
...
occ.data <- list(n=n, nzeroes=nzeroes, J=J, K=K, X=Xaug, elev=ele, pop=pop, punish=punish, reach=reach,
                 income=income,camhour = camhour, cam_angle=cam_angle, park.ind=park.ind, nsite=nsite)

X <-Xaug
elev <- ele
dist<- distance
occ.data.qou <- list('n', 'nzeroes', 'J', 'K', 'X', 'elev', 'pop', 'punish', 'reach',
                     'income','camhour' , 'cam_angle', 'park.ind','nsite')
...
modelFile='R/model_5_var_site_effe.txt'
...
occ.inits = function() {
  ...
  list(
    ...
    a1=rnorm(n+nzeroes), 
    a3=rnorm(n+nzeroes), 
    a5=rnorm(n=n+nzeroes, mean = 8.4, sd=7.9),a6=rnorm(n=n+nzeroes, mean = 1.7, sd=1.1),
  #  a5=rnorm(n+nzeroes), a6=rnorm(n+nzeroes),
    a11=rnorm(n+nzeroes),a.park=matrix(rnorm(nsite*(n+nzeroes)), ncol=nsite) 
  )           
}
##
...
names(›) <- c("coef","name","mean","lower","upper")
...
## fast way
caterplot(fit, c("mu.a1","mu.a3","mu.a5","mu.a6","mu.a11","mu.b1","mu.b2"))
caterplot(fit, c("sigma.park"))
caterplot(fit, c("mu.a1","mu.a3","mu.a5","mu.a6","mu.a11","mu.b1","mu.b2","park"), 
          labels=c("elevation","population","punishment","outreach","income","camhour","camangle","park1","park2","park3","park4","park5","park6"), reorder=FALSE)
         
caterplot(fit, c("a1"), reorder=FALSE)

caterplot(fit, c("mu.a1","mu.a3","mu.a5","mu.a6","mu.a11","mu.b1","mu.b2"), 
          labels=c("elevation","population","punishment","outreach","income","camhour","camangle"),reorder=FALSE)
