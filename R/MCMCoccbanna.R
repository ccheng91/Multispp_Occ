...
fitppp <- jags.parallel(occ.data.qou, occ.inits, occ.params, modelFile,     ## 
           n.chains=3, n.iter=50, n.burnin=30, n.thin=1, n.cluster = 3, export_obj_names =c('Z.int') )
...
caterplot(fit, c("mu.a1","mu.a2","mu.a3","mu.a4"))
...
for (j in 1:length(r.dummy)) {
  for(i in 1: length(spp)) {
    for (k in 1:dim(u)[1]) {
      psi[i,k] <- plogis(u[k,i] +a1[k,i]*mean(ele) +a2[k,i]*mean(pop) + a3[k,i]*mean(punish) + a4[k,i]*r.dummy[j] )  
    } 
  }
