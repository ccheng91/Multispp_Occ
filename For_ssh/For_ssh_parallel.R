...
### 3-demensinal data
####
setwd("") # change path~~~~~# change path~~~~~# change path~~~~~# change path~~~~~
rm(list=ls(all=TRUE))
list.filenames <- list.files(path = "data/All_widedata", pattern="*.csv") # change path~~~~~# change path~~~~~
list.filenames <- paste(c("data/All_widedata/"), list.filenames, sep="")# change path~~~~~# change path~~~~~
...
sitecov <- read.csv("data/sitecov_temp_Cheng_reorded.csv") # change path~~~~~# change path~~~~~
days <- sitecov$days
days.5 <- days%/%5
daysd.5<- days%%5
...
#Specify the data
occ.data <- list(n=n, nzeroes=nzeroes, J=J, K=K, X=Xaug, elev=ele, dist=distance,pop=pop,pasize=pasize, punish=punish, reach=reach,
                 relationship=relationship,score.P=score.P,score.H=score.H,edu=edu,income=income,camhour = camhour, cam_angle=cam_angle)
...
occ.data.qou <- list('n', 'nzeroes', 'J', 'K', 'X', 'elev', 'dist','pop','pasize', 'punish', 'reach',
                     'relationship','score.P','score.H','edu','income','camhour' , 'cam_angle')
...
occ.params <- c('u', 'v', 'w', 
                ...
                'mu.a1', 'mu.a2','mu.a3','mu.a4', 'mu.a5', 'mu.a6','mu.a7','mu.a8', 'mu.a9', 'mu.a10','mu.a11', 
                'mu.b1', 'mu.b2',
...
modelFile='R/MSMSoccbanna.txt' # change path~~~~~# change path~~~~~# change path~~~~~
...
occ.inits = function() {
  ...
  list(
    ...
    a7=rnorm(n+nzeroes), a8=rnorm(n+nzeroes),
    a9=rnorm(n+nzeroes), a10=rnorm(n+nzeroes),a11=rnorm(n+nzeroes) 
  )           
}
...
fit.par <- jags.parallel(occ.data.qou, occ.inits, occ.params, modelFile,     ## run 5 chains on different core 
                        n.chains=5, n.iter=80000, n.burnin=50000, n.thin=20, n.cluster = 5, export_obj_names =c('Z.int') )
...
save.image("~/Desktop/data/Multispecies_occupancy/Workplace/Globalmodel_3_50000_5000_50.RData") # change path~~~~~# change path~~~~~
